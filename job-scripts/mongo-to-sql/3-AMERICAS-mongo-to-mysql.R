...
# connect to MongoDB
m_db <- mongo(url = "mongodb://balco:n0nLadimentico@localhost:27017/admin", collection = "lor_match_info_na") # db with match to prepare
...
already_in_db <- tbl(con, "lor_match_info_na") %>% 
  select(match_id) %>% 
  distinct() %>% 
  collect() %>% 
  pull()
