# Creates a MySQL table with archetype informations for the current patch (from data of MySQL databases)
...
data_v2 <- tbl(con, "lor_match_info_v2") %>%
  ...
  count(region, archetype, game_outcome, time_frame, is_master) %>% 
  collect()
...
if(nrow(data_v2) >  0){
  ...
  data_v2 %>% 
    DBI::dbWriteTable(conn = con, name = "ranked_archetypes", value = ., overwrite = TRUE, row.names = FALSE) 
