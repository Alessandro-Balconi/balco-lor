...
# connect to db
m_match   <- mongo(url = "mongodb://balco:n0nLadimentico@localhost:27017/admin", collection = "lor_match_info_na")
m_player  <- mongo(url = "mongodb://balco:n0nLadimentico@localhost:27017/admin", collection = "lor_player_na")

# 3. set api parameters ----

# API path
base.url           <- "https://americas.api.riotgames.com/" # americas, asia, europe, sea
  ...
  if(i == 1){
    ...
    if(get_leaderboard$status_code == 200){
      ...
      if(length(master_players) >= 50){
        ...
        saveRDS(object = puuid_list, file = "/home/balco/dev/lor-meta-report/templates/master_leaderboards/americas.rds")
        ...
        saveRDS(object = puuid_list, file = "/home/balco/dev/lor-meta-report/templates/master_leaderboards/old_americas.rds")
        
      } else if(file.exists("/home/balco/dev/lor-meta-report/templates/master_leaderboards/old_americas.rds")){
        
        # print message to console
        cat(sprintf(" - There are only %s master players; using .rds with previous season' masters. \n", length(master_players)))
        
        # last season master players
        old_master_players <- readRDS("/home/balco/dev/lor-meta-report/templates/master_leaderboards/old_americas.rds")
