...
while(TRUE){
  ...
  if(i == 1){
    ...
    if(get_leaderboard$status_code == 200){
      ...
      } else if(file.exists("/home/balco/dev/lor-meta-report/templates/master_leaderboards/europe.rds")){
        
        # print message to console
        cat(sprintf(" - There are only %s master players; using .rds with previous season' masters. \n", length(master_players)))
        
        # last season master players
        old_master_players <- readRDS("/home/balco/dev/lor-meta-report/templates/master_leaderboards/europe.rds")
